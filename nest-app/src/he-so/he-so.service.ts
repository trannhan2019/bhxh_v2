...
export class HeSoService {
  ...
  async listByLoai() {
    const all = await this.prisma.heSo.findMany();
    const chucVu = all.filter((item) => item.loai === 'CHUC_VU');
    const trachNhiem = all.filter((item) => item.loai === 'TRACH_NHIEM');
    return { chucVu, trachNhiem };
  }
}
